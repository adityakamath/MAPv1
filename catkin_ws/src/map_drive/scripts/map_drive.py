...
class driver:
	...
	# get cmd_vel message, and get linear/angular velocity
	def get_cmd_vel(self, data):
		x = data.linear.x
		angular = data.angular.z
		self.send_cmd_to_arduino(x, angular)

	# translate x/angular to PWM signals of each wheel and send to arduino
	def send_cmd_to_arduino(self, x, angular):
		# calculate right and left wheels' signal
		right = int((x + angular)*100)
		left = int((x - angular)*100)
		# format for arduino
